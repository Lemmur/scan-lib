...
const TreeComponent = () => {
    ...
    const onDrop = (info) => {
        ...
        console.log('moveNode', info.dragNode.id, info.node.parentId, info);
        ...
        if (dropToGap) {
            ...
            prevId = node.id;
            nextId = node.nextId;
            console.log('dropToGap', {dragNodeIds, parentId, prevId, nextId});
        } else {
            parentId = node.id;
            prevId = 0;
            nextId = node.children?.length ? node.children[0].id : 0;
            console.log('dropToNode', {dragNodeIds, parentId, prevId, nextId});
        }
    ...
    const renderTreeNode = (treeNode: TNode = tree, pNumber = 1) => {
        return <TreeNode
        ...
        >
            { treeNode.children?.map((child, index) => renderTreeNode(child, index + 1)) }
        </TreeNode>;
    };
